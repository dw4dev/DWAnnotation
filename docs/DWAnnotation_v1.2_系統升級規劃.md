...
### Phase 2 — 設定視窗重構 + 工具列優化
> 估計工時：1~2 session

**目標**：現代化設定架構，支援後續新設定項擴充；工具列智慧縮小。
...
- [ ] §5.1 雷射筆指示器
  ...
  - 工具列新增「雷射筆」按鈕
- [ ] §5.2 聚光燈功能
  - 全螢幕半透明遮罩（80% 黑），中央裁剪圓形或正方形明亮區
  - §5.2.1 設定：形狀（Circle/Square）、大小（半徑 px）
...
- 編輯器 Undo/Redo 採 Command Pattern（`IEditCommand` 介面）
...
public string ScreenshotFormat { get; set; } = "PNG"; // PNG / JPG / BMP
...
│   └── DarkTheme.xaml
├── Views/
│   ├── MainToolbarWindow.xaml       [MODIFY] - 主題套用、工具列縮小
...
│   └── OverlayWindow.xaml           [MODIFY] - 新增雷射筆/聚光燈層
├── ViewModels/
...
└── Services/
    ├── ScreenshotService.cs         [NEW] - 截圖邏輯集中
